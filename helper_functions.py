...
def sequential_Newton(guesses, eigs, min_eigenvalue, max_eigenvalue, printerval=np.inf):
    """Search for closeish convergence. Then search for full convergence. 
    Then clean solutions (get rid of neg, too large, nans)"""
    solutions = []
    for guess in guesses:
        solution = eigs(guess, solve_type="SVD iterate", printerval=printerval, tol=1e-4, max_steps=20)
...
class coeff_interp:
    ...
    def __init__(self,data,radius,degree):
        A = np.fft.rfft(data,axis=1)/data.shape[1]
        self.data = {}
        self.modes = np.arange(data.shape[1]/2+1).astype(np.int32)
        A[:,1:] *= 2
        for k in self.modes: 
